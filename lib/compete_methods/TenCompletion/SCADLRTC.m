...
function [X, errList,lowrank_part] = SCADLRTC(T, Omega, alpha, beta, maxIter, epsilon, X)
...
%      Sigma_old{i} = max(1-Sigma_old{i}/gamma1,0);
     Sigma_old{i}=lambda*(Sigma_old{i}<=1) + max(a*lambda-Sigma_old{i}, 0)/(a-1)*(Sigma_old{i}>1);
end
%% one-step ALM
...
for k = 1: maxIter
%     if mod(k, 20) == 0
%         fprintf('ScadLRTC: iterations = %d   difference=%f\n', k, errList(k-1));
%     end
    beta = beta * 1.05;
...
errList = errList(1:k);
% fprintf('ScadLRTC ends: total iterations = %d   difference=%f\n\n', k, errList(k));
