...
if nargin < 7
    X = T;
    X(logical(1-Omega)) = mean(T(Omega)); % initilize the unobserved values
...
for k = 1: maxIter
%     if mod(k, 20) == 0
%         fprintf('HaLRTC: iterations = %d   difference=%4.4f\n', k, errList(k-1));
%     end
    beta = beta * 1.05;
...
errList = errList(1:k);
% fprintf('HiLRTC ends: total iterations = %d   difference=%4.4f\n\n', k, errList(k));
